...
function ControlTower() {
  ...
  const metrics = useMemo(() => {
    ...
    const pendingCouriers = couriers.filter((c) => !c.isApproved);
    ...
    const stuckOrders = orders.filter(
      (o) => o.status === "ACCEPTED" || o.status === "ARRIVED_PICKUP",
    );
  ...
  return (
    <div className="enterprise">
      ...
      <div className="footer-insight">
        <p>
          System is processing {metrics.activeOrders} active deliveries across{" "}
          {metrics.totalCouriers} couriers.
        </p>
      </div>
    </div>
  );
