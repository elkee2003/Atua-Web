...
export default function PayoutUpdateForm(props) {
  const {
    id: idProp,
    payout: payoutModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    ...
    setTransferCode(cleanValues.transferCode);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Payout, idProp)
        : payoutModelProp;
      setPayoutRecord(record);
    };
    queryData();
  }, [idProp, payoutModelProp]);
  React.useEffect(resetStateValues, [payoutRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Payout.copyOf(payoutRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PayoutUpdateForm")}
      {...rest}
    >
      <TextField
        label="Courier id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || payoutModelProp)}
